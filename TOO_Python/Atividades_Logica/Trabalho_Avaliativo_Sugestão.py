...
for indice in range(quantidade_mensagens) :
    ...
    for letra in texto:
        ...
        elif letra in alfabeto_maiusculas:
                nova_posicao = (alfabeto_maiusculas.find(letra) + posicoes_deslocar) % 26
                texto_criptografado += alfabeto_maiusculas[nova_posicao]
        elif letra in numeros:
                nova_posicao = (numeros.find(letra) + posicoes_deslocar) % 10
                texto_criptografado += numeros[nova_posicao]
        elif letra == ' ':
